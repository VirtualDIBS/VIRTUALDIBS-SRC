...
uint256 hashGenesisBlock("0x91184c5fb44d4ddd6ffc0fad26ee7e9f382148e532d2152a6e13544241832141");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // VIRTUALDIBSCOIN: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "VIRTUALDIBSCOIN Signed Message:\n";
...
{
    // VIRTUALDIBSCOIN: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // VIRTUALDIBSCOIN: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // VIRTUALDIBSCOIN: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // VIRTUALDIBSCOIN: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x91184c5fb44d4ddd6ffc0fad26ee7e9f382148e532d2152a6e13544241832141");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1418834319;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086756506;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // virtualdibs: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("VIRTUALDIBSCOINMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("VIRTUALDIBSCOINMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static virtualdibsMiner(CWallet *pwallet)
{
    printf("VIRTUALDIBSCOINMiner:\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("virtualdibs-miner");
    ...
    try { loop {
        ...
        printf("Running VIRTUALDIBSCOIN with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("VIRTUALDIBSCOINMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&virtualdibsMiner, pwallet));
}
